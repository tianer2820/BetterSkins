...
ColorRampBar::ColorRampBar(wxWindow *parent, wxWindowID id) : wxWindow(parent, id)
{
...
void ColorRampBar::remove(int index)
{
    if (index == -1)
    {
...
RGBColor ColorRampBar::mapColor(int value)
{
    return getColorAt(value);
}

vector<RGBColor> ColorRampBar::getColorList()
{
    return color_list;
}
vector<int> ColorRampBar::getPosList()
{
    return position_list;
};

void ColorRampBar::init(vector<RGBColor> colors, vector<int> poses)
{
    color_list = colors;
    position_list = poses;
}

int ColorRampBar::insertKnob(int pos, RGBColor color)
{
...
RGBColor ColorRampBar::getColorAt(int pos)
{
    if (color_list.size() < 1)
    {
...
RGBColor ColorRampBar::mixRGB(RGBColor c0, RGBColor c1, double percent)
{
    int rgb0[3];
    int rgb1[3];
    c0.getRGB(rgb0);
...
void ColorRampBar::onPaint(wxPaintEvent &event)
{
    wxRect bar_area;
    bar_area.x = r;
    bar_area.y = r;
...
void ColorRampBar::onSize(wxSizeEvent &event)
{
...
void ColorRampBar::onMouse(wxMouseEvent &event)
{
    int x = event.GetX();
    int y = event.GetY();
...
void ColorRampBar::onColorPicker(wxCommandEvent &event)
{
...
void ColorRampBar::moveKnob(int index, int value)
{
...
int ColorRampBar::posToKnobIndex(int x, int y)
{
    for (int i = 0; i < position_list.size(); i++)
    {
...
void ColorRampBar::sendRampChangeEvent()
{
...
wxSize ColorRampBar::DoGetBestSize() const
{
